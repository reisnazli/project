function  [w,w_mod,w_sm,w_spec]  =  analysisforind(f, J, alpha, tau)
%
...
for depth = 1 : J
   ...
   % complexify
   w_sm{depth} = abs(real(ifft(Z1,M/2)).^2 + sqrt(-1) * (real(ifft(Z2,M/2))).^2);
   w_spec{depth} = abs(real(ifft(Z1,M/2)).^2 + sqrt(-1) * (real(ifft(Z2,M/2))).^2);
   w{depth} = abs(real(ifft(Z1,M/2)).^2 + sqrt(-1) * (real(ifft(Z2,M/2))).^2);
   w_mod{depth}=abs(real(ifft(Z1,M/2))).^2+(real(ifft(Z2,M/2)).^2);
