...
online.model<-lm(shares ~ kw_max_avg + LDA_02 + data_channel_is_world + is_weekend + data_channel_is_socmed + weekday_is_saturday + LDA_04 + data_channel_is_entertainment + data_channel_is_tech + kw_max_avg + weekday_is_sunday + LDA_00 + num_hrefs + global_subjectivity + kw_min_avg + global_sentiment_polarity + rate_negative_words + kw_min_min + title_subjectivity + LDA_01, data = online.data)

online.model.predict<-lm(shares ~ kw_max_avg + LDA_02 + data_channel_is_world + is_weekend + data_channel_is_socmed + weekday_is_saturday + LDA_04 + data_channel_is_entertainment + data_channel_is_tech + kw_max_avg + weekday_is_sunday + LDA_00 + num_hrefs + global_subjectivity + kw_min_avg + global_sentiment_polarity + rate_negative_words + kw_min_min + title_subjectivity + LDA_01, data = training)
